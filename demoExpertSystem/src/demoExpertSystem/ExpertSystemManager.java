...
// implemented from example in Chapter 2 of "Artificial Intelligence - a guide to intelligent systems" 
// by Michael Negnevitsky
//
// Uses facts in a database (here set in FactFiller class, and through user input)
// and rules (here set in RuleFiller class)
// to evaluate unknown fact, name of which is given by the user
//
// If expanded, could allow custom rules and fact fields to be entered, therefore acting as an expert system shell
// Currently supports rules with multiple antecedents and one single consequent, with positive relationships.
...
public class ExpertSystemManager {
	...
	// set some fact values automatically without asking user
